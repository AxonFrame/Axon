 ...
 * @brief constructor
 *
 * @param thread global unique  of the thread for later identification
 */
Thread::Thread(const std::string& thread)
    : m_thread(thread), m_threadId(ThreadHandler::getInstance()->getNewId())
{
}
...
Thread::~Thread()
{
    ThreadHandler::getInstance()->unregisterThread(m_thread, m_threadId);
    stopThread();
    clearEventQueue();
