 ...
 *                    subdirectories
 * @param exceptions list with directory-s, which should be skipped
 */
void
        ...
        if (is_directory(itr->path())) {
            if (withSubdirs == true) {
                ...
                for (const std::string& exception : exceptions) {
                    if (itr->path().file().string() == exception) {
 ...
 * @param exceptions list with directory-s, which should be skipped (Default: empty list)
 *
 ...
 */
bool
reFileOrDir(const std::filesystem::path& oldPath,
                ...
                ErrorContainer& error)
{
    ...
    std::filesystem::re(oldPath, newPath, errorCode);
