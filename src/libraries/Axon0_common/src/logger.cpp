...
initFileLogger(const std::string& directoryPath,
               ...
               const bool debugLog)
{
    return Logger::m_logger->initFileLogger(directoryPath, baseFile, debugLog);
}
...
{
    return Axon::Logger::m_logger->logData(message, "INFO", color);
}
                       ...
                       const bool debugLog)
{
    ...
    m_filePath = m_directoryPath + "/" + m_baseFile + ".log";
