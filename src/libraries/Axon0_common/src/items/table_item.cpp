 ...
 *
 * @param internal  for internal indentification of the columns inside the body
 * @param shown  which is shown in the string-output of the table
 *                  if leaved blank, the  is set equal to the internal-
 *
 * @return should return true everytime
 */
bool
TableItem::addColumn(const std::string& internal, const std::string& shown)
{
...
/**
 * @brief re a column in the header of the table
 *
 * @param internal  for internal indentification of the columns inside the body
 * @param newShown new  for string output
 *
 * @return false if internal- doesn't exist, else true
 */
bool
TableItem::reColume(const std::string& internal, const std::string& newShown)
{
    const uint64_t size = m_header.size();

    for (uint64_t x = 0; x < size; x++) {
        if (m_header[x]["inner"] == internal) {
            m_header[x]["outer"] = newShown;
            return true;
        }
    }
 ...
 * @brief delelete a colume from the table
 *
 * @param internal internal  of the column
 *
 * @return false if internal  doesn't exist, else true
 */
bool
TableItem::deleteColumn(const std::string& internal)
{
    const uint64_t size = m_header.size();
...
/**
 * @brief get table header, but only the inner s
 *
 * @return copy of table-header
 */
json
...
/**
 * @brief get all internal column-s
 *
 * @return list with all internal s
 */
