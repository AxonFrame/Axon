...
class TableItem
{
   public:
    ...
    // column
    bool addColumn(const std::string& internal, const std::string& shown = "");
    bool reColume(const std::string& internal, const std::string& newShown);
    bool deleteColumn(const uint64_t x);
    bool deleteColumn(const std::string& internal);
   ...
   private:
    ...
    const std::vector<std::string> getInner();
