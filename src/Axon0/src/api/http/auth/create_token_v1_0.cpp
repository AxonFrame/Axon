    ...
    : Blossom("Create a JWT-access-token for a specific user.", false)
{
    ...
    registerOutputField("", SAKURA_STRING_TYPE).setComment(" of the user.");
                         ...
                         Axon::ErrorContainer& error)
{
    ...
    Axon::generate_SHA_256(compareHash, saltedPw);
    ...
    if (success == false) {
        error.addMessage("Could not read 'token_expire_time' from config of Axon.");
        status.statusCode = INTERNAL_SERVER_ERROR_RTYPE;
    }
    ...
    const std::string jwtToken
        = jwt::create()
              ...
              .sign(jwt::algorithm::hs256{(const char*)AxonRoot::tokenKey.data()});

    blossomIO.output["id"] = userId;
    blossomIO.output["is_admin"] = userData.isAdmin;
    blossomIO.output[""] = userData.;
