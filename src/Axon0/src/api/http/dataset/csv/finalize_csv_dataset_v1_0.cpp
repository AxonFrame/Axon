                                ...
                                Axon::ErrorContainer& error)
{
    ...
    // write data to file
    if (convertCsvData(result.location, result., inputBuffer, error) == false) {
        status.statusCode = INTERNAL_SERVER_ERROR_RTYPE;
                                       ...
                                       Axon::ErrorContainer& error)
{
    ...
    Axon::splitStringByDelimiter(lines, stringContent, '\n');
    ...
    for (uint64_t lineNum = 0; lineNum < lines.size(); lineNum++) {
        ...
        Axon::splitStringByDelimiter(lineContent, *line, ',');
        ...
        if (isHeader) {
            ...
            for (const std::string& col : lineContent) {
                ...
                description[col] = inputDescrEntry;
                counter++;
            }
            isHeader = false;
