    ...
    : Blossom("Compare a list of values with a dataset to check accuracy.")
{
    ...
    registerInputField("column_", SAKURA_STRING_TYPE)
        .setComment(" of the column to read from the dataset.")
        .setRegex(_REGEX);
                                    ...
                                    Axon::ErrorContainer& error)
{
    ...
    const std::string column = blossomIO.input["column_"];
    ...
    if (datasetFileHandle.description.contains(column) == false) {
        status.errorMessage = "Column with  '" + column
    ...
    // set file-selectors
    const json range = datasetFileHandle.description[column];
