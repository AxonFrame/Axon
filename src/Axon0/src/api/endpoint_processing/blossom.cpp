 ...
 *
 * @param   of the filed to identifiy value
 * @param fieldType type for value-validation
 *
 * @return reference to entry for further updates
 */
FieldDef&
Blossom::registerInputField(const std::string& , const FieldType fieldType)
{
    errorCodes.push_back(BAD_REQUEST_RTYPE);
    auto ret = m_inputValidationMap.try_emplace(, FieldDef(FieldDef::INPUT_TYPE, fieldType));
 ...
 */
FieldDef&
Blossom::registerOutputField(const std::string& , const FieldType fieldType)
{
    auto ret = m_outputValidationMap.try_emplace(, FieldDef(FieldDef::OUTPUT_TYPE, fieldType));
    return ret.first->second;
}
...
{
    for (const auto& [, field] : m_inputValidationMap) {
        if (field.defaultVal != nullptr) {
            if (values.contains() == false) {
                values[] = field.defaultVal;
            }
        }
    }
}
                                    ...
                                    std::string& errorMessage)
{
    if (allowUnmatched == false) {
        ...
        for (const auto& [, _] : input.items()) {
            if (validationMap.find() == validationMap.end()) {
                // build error-output
    ...
    for (const auto& [, field] : validationMap) {
 ...
 */
void
Blossom::createError(const BlossomIO& blossomIO, Axon::ErrorContainer& error)
{
    Axon::TableItem errorOutput;
