 ...
 * @brief constructor
 */
HttpWebsocketThread::HttpWebsocketThread(const std::string& thread) : Thread(thread) {}
...
{
    while (m_abort == false) {
        tcp::socket* socket = AxonRoot::httpServer->getSocket();
        if (socket != nullptr) {
            handleSocket(socket);
            delete socket;
        }
...
HttpWebsocketThread::handleSocket(tcp::socket* socket)
{
    ...
    if (websocket::is_upgrade(httpRequest)) {
        ...
        // process request
        processResult = AxonRoot::httpServer->httpProcessing->processRequest(
...
HttpWebsocketThread::processInitialMessage(const std::string& message, std::string& errorMessage)
{
    ...
    // forward connection to shiori or Axon
