...
const std::string
createErrorMessage(const std::string& , const FieldType fieldType)
{
    std::string err = "Value-validation failed, because a item '" +  + "'  has the false type:";
    switch (fieldType) {
    ...
    for (const auto& [, field] : defs) {
        ...
        if (values.contains()) {
            ...
            if (checkType(item, field.fieldType) == false) {
                errorMessage = createErrorMessage(, field.fieldType);
                return false;
            }
            ...
            if (field.regex.size() > 0) {
                ...
                if (std::regex_match(std::string(values[]), re) == false) {
                    errorMessage = "Given item '" +  + "' doesn't match with regex \"^"
                                   + field.regex + "$\"";
                    return false;
                }
            }
            ...
            if (field.upperLimit != 0 || field.lowerLimit != 0) {
                if (item.is_number_integer()) {
                    ...
                    if (value < field.lowerLimit) {
                        errorMessage = "Given item '" +  + "' is smaller than "
                                       + std::to_string(field.lowerLimit);
                        return false;
                    }

                    if (value > field.upperLimit) {
                        errorMessage = "Given item '" +  + "' is bigger than "
                ...
                if (item.is_string()) {
                    ...
                    if (length < field.lowerLimit) {
                        errorMessage = "Given item '" +  + "' is shorter than "
                                       + std::to_string(field.lowerLimit) + " characters";
                        return false;
                    }

                    if (length > field.upperLimit) {
                        errorMessage = "Given item '" +  + "' is longer than "
            ...
            if (field.match != nullptr) {
                if (field.match != item) {
                    errorMessage = "Item '" +  + "' doesn't match the the expected value:\n   ";
