...
Axon::GpuInterface* AxonRoot::gpuInterface = nullptr;
AxonRoot* AxonRoot::root = nullptr;
PhysicalHost* AxonRoot::physicalHost = nullptr;
HttpServer* AxonRoot::httpServer = nullptr;
std::string AxonRoot::tokenKey{};

/**
 * @brief constructor
 */
AxonRoot::AxonRoot() { root = this; }
 ...
 */
bool
AxonRoot::init(Axon::ErrorContainer& error)
{
    srand(time(NULL));
 ...
 */
bool
AxonRoot::initDatabase(Axon::ErrorContainer& error)
{
    bool success = false;

    // read database-path from config
    Axon::SqlDatabase* database = Axon::SqlDatabase::getInstance();
...
AxonRoot::initHttpServer()
{
    ...
    for (uint32_t i = 0; i < numberOfThreads; i++) {
        const std::string  = "HttpWebsocketThread";
        HttpWebsocketThread* httpWebsocketThread = new HttpWebsocketThread();
...
/**
 * @brief AxonRoot::initDataDirectory
 * @return
 */
bool
AxonRoot::initDataDirectory(Axon::ErrorContainer& error)
{
    bool success = false;
 ...
 */
bool
AxonRoot::initPolicies(Axon::ErrorContainer& error)
{
    bool success = false;
    ...
    if (Axon::readFile(policyFileContent, policyFilePath, error) == false) {
        error.addMessage("Failed to read policy-file");
        return false;
    }

    // parse policy-file
    Axon::Policy* policies = Axon::Policy::getInstance();
 ...
 */
bool
AxonRoot::initJwt(Axon::ErrorContainer& error)
{
    bool success = false;
    ...
    if (Axon::readFile(tokenKeyString, tokenKeyPath, error) == false) {
 ...
 */
void
AxonRoot::clearCluster(Axon::ErrorContainer& error)
{
