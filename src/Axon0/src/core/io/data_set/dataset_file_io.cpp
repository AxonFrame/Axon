                ...
                Axon::ErrorContainer& error)
{
    ...
    if (fileHandle.writeBuffer.usedBufferSize + inputSize > fileHandle.writeBuffer.totalBufferSize)
    {
        ...
        if (inputSize > fileHandle.writeBuffer.totalBufferSize) {
            ...
            Axon::addData_DataBuffer(fileHandle.writeBuffer, input, inputSize);
        }
    }
    else {
        Axon::addData_DataBuffer(fileHandle.writeBuffer, input, inputSize);
                ...
                Axon::ErrorContainer& error)
{
    ...
    // open file
    result.targetFile = new Axon::BinaryFile(filePath);
    ...
    Axon::DataBuffer descriptionBuffer(Axon::calcBytesToBlocks(result.header.descriptionSize));
                   ...
                   Axon::ErrorContainer& error)
{
    ...
    // set  in header
    if (result.header..set() == false) {
        error.addMessage("New data-set  '" +  + "' is invalid");
        return INVALID_INPUT;
    }
                   ...
                   Axon::ErrorContainer& error)
{
    ...
    Axon::BinaryFile targetFile(filePath);
