 ...
 */
ReturnStatus
BufferIO::writeClusterIntoBuffer(Axon::DataBuffer& target,
 ...
 */
bool
BufferIO::initializeTarget(const uint64_t size, Axon::ErrorContainer& error)
{
    if (Axon::reset_DataBuffer(*m_buffer, Axon::calcBytesToBlocks(size)) == false) {
 ...
 */
bool
BufferIO::writeFromLocalBuffer(const LocalBuffer& localBuffer, Axon::ErrorContainer&)
{
 ...
 */
bool
BufferIO::readToLocalBuffer(LocalBuffer& localBuffer, Axon::ErrorContainer&)
{
    uint8_t* u8Data = static_cast<uint8_t*>(m_buffer->data);
