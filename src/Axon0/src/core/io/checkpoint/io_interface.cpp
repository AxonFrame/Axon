 ...
 */
ReturnStatus
IO_Interface::serialize(const Cluster& cluster, Axon::ErrorContainer& error)
{
                          ...
                          Axon::ErrorContainer& error)
{
    ...
    for (uint64_t i = 0; i < numberOfHexagons; i++) {
        ...
        if (host != nullptr) {
            ...
            cluster.hexagons[i].attachedHost = AxonRoot::physicalHost->getFirstHost();
        }
 ...
 */
ReturnStatus
IO_Interface::serialize(const Hexagon& hexagon, Axon::ErrorContainer& error)
{
    // hexagon-entry
    ...
    SynapseBlock* synapseBlocks
        = Axon::getItemData<SynapseBlock>(hexagon.attachedHost->synapseBlocks);
    ...
    if (hexagon.inputInterface != nullptr) {
        ...
        if (inputEntry..set(hexagon.inputInterface->) == false) {
    ...
    if (hexagon.outputInterface != nullptr) {
        ...
        if (outputEntry..set(hexagon.outputInterface->) == false) {
            return INVALID_INPUT;
        }
 ...
 */
ReturnStatus
IO_Interface::deserialize(Hexagon& hexagon, uint64_t& positionPtr, Axon::ErrorContainer& error)
{
    ...
    if (hexagonEntry.inputInterfacesPos != 0) {
        ...
        auto ret = hexagon.cluster->inputInterfaces.try_emplace(inputIf., inputIf);
        if (ret.second == false) {
            error.addMessage("Input-data invalid");
            return INVALID_INPUT;
        }

        hexagon.inputInterface = &hexagon.cluster->inputInterfaces[inputIf.];
    }
    ...
    if (hexagonEntry.outputsInterfacesPos != 0) {
        ...
        auto ret = hexagon.cluster->outputInterfaces.try_emplace(outputIf., outputIf);
        if (ret.second == false) {
            error.addMessage("Input-data invalid");
            return INVALID_INPUT;
        }

        hexagon.outputInterface = &hexagon.cluster->outputInterfaces[outputIf.];
    }
