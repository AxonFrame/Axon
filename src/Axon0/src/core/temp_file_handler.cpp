...
TempFileHandler::TempFileHandler() : Axon::Thread("tempfile-handler") {}
 ...
 * @param   of the file for easier identification
                             ...
                             Axon::ErrorContainer& error)
{
    ...
    Axon::BinaryFile* tempfile = nullptr;
    do {
        // allocate storage on disc
        tempfile = new Axon::BinaryFile(targetFilePath);
        ...
        Axon::UploadFileHandle fileHandle;
        fileHandle.userContext = userContext;
        fileHandle.file = tempfile;
        fileHandle.bitBuffer = new Axon::BitBuffer(numberOfInputSegments);
 ...
 */
Axon::UploadFileHandle*
TempFileHandler::getFileHandle(const std::string& uuid, const Axon::UserContext& context)
{
    const std::lock_guard<std::mutex> lock(m_fileHandleMutex);
                              ...
                              const uint64_t size)
{
    ...
    // write data to file
    Axon::BinaryFile* ptr = it->second.file;
 ...
 */
bool
TempFileHandler::getData(Axon::DataBuffer& result, const std::string& uuid)
{
    const std::lock_guard<std::mutex> lock(m_fileHandleMutex);
                                ...
                                Axon::ErrorContainer& error)
{
    ...
    if (Axon::deleteFileOrDir(targetFilePath, error) == false) {
                          ...
                          Axon::ErrorContainer& error)
{
    ...
    if (it != m_tempFiles.end()) {
        ...
        if (Axon::reFileOrDir(tempfileLocation, targetLocation, error) == false) {
