 ...
 */
void
initializeSettings(Cluster* cluster, const Axon::ClusterMeta& clusterMeta)
{
...
initializeInputs(Cluster* cluster, const ClusterMeta& clusterMeta)
{
    for (const InputMeta& inputMeta : clusterMeta.inputs) {
        ...
        inputInterface. = inputMeta.;

        cluster->inputInterfaces.try_emplace(inputMeta., inputInterface);
        ...
        targetHexagon->inputInterface = &cluster->inputInterfaces[inputMeta.];
    }
}
...
initializeOutputs(Cluster* cluster, const ClusterMeta& clusterMeta)
{
    for (const OutputMeta& outputMeta : clusterMeta.outputs) {
        ...
        outputInterface. = outputMeta.;
        outputInterface.type = outputMeta.type;

        cluster->outputInterfaces.try_emplace(outputMeta., outputInterface);
        ...
        cluster->hexagons[outputMeta.targetHexagonId].outputInterface
            = &cluster->outputInterfaces[outputMeta.];
    }
}
 ...
 */
void
initializeHexagons(Cluster* cluster, const Axon::ClusterMeta& clusterMeta, LogicalHost* host)
{
    ...
    for (uint32_t i = 0; i < clusterMeta.hexagons.size(); i++) {
        ...
        if (host != nullptr) {
            ...
            newHexagon->attachedHost = AxonRoot::physicalHost->getFirstHost();
        }
 ...
 */
void
updateAxons(Cluster* cluster, const Axon::ClusterMeta& clusterMeta)
{
...
{
    const Axon::Position next = getNeighborPos(sourceHexagon.header.hexagonPos, side);
    // debug-output
