...
WorkerThread::WorkerThread() : Axon::Thread("WorkerThread") {}
 ...
 */
void
WorkerThread::handleTask(const Axon::WorkerTask& task)
{
...
{
    while (m_abort == false) {
        Axon::WorkerTask task = m_host->getWorkerTaskFromQueue();
