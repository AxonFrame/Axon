 ...
 */
bool
PhysicalHost::init(Axon::ErrorContainer& error)
{
    ...
    if (Axon::Host::getInstance()->initHost(error) == false) {
        return false;
    }

    // identify and init cpu's
    const uint32_t numberOfSockets = Axon::Host::getInstance()->cpuPackages.size();
