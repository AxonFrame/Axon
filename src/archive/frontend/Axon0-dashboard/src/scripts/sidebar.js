...
function initAllDropdowns() 
{
    ...
    let dropdown = document.getElementsByClass("sidebar_drop_down");
    for(let i = 0; i < dropdown.length; i++) 
    {
        dropdown[i].addEventListener("click", function() 
        {
            ...
            for(let j = 0; j < dropdown.length; j++) 
            {
                ...
                dropdown[j].class = dropdown[j].class.replace(" active", "");
            }
    ...
    let dropdownEntries = document.getElementsByClass("sidebar_dropdown_entry");
    for(let i = 0; i < dropdownEntries.length; i++) 
    {
        dropdownEntries[i].addEventListener("click", function() 
        {
            ...
            for(let j = 0; j < dropdownEntries.length; j++) {
                dropdownEntries[j].class = dropdownEntries[j].class.replace(" active", "");
            }
...
function activateDefaultSideEntry() 
{
    ...
    for(let i = 0; i < dropdown.length; i++) 
    {
        ...
        dropdown[i].class = dropdown[i].class.replace(" active", "");
    }

    dropdown[defaultDropdownIndex].nextElementSibling.style.maxHeight = "200px"; 
    dropdown[defaultDropdownIndex].class += " active";

    // mark initial dropdown-entry
    let dropdownEntries = document.getElementsByClass("sidebar_dropdown_entry");
    for(let i = 0; i < dropdownEntries.length; i++) {
        dropdownEntries[i].class = dropdownEntries[i].class.replace(" active", "");
    }

    dropdownEntries[defaultDropdownIndex].class += " active";
}
...
function resetAllSidebarEntries() 
{
    ...
    let tabcontent = document.getElementsByClass("tabcontent");
    for(let i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }

    // Get all elements with class="tablinks" and remove the class "active"
    let tablinks = document.getElementsByClass("tablinks");
