...
{
    LOG_DEBUG("created new gpu-interface for OpenCL device: " + device.getInfo<CL_DEVICE_>());
...
GpuInterface::initCopyToDevice(GpuData& data, ErrorContainer& error)
{
    ...
    for (auto& [, workerBuffer] : data.m_buffer) {
        // skip values
        ...
        {
            error.addMeesage("failed to copy data to device, because buffer with  '" + 
...
{
    LOG_DEBUG("add kernel with id: " + kernel);
    ...
    def.kernel = cl::Kernel(program, kernel.c_str());

    auto ret = data.m_kernel.try_emplace(kernel, def);
    if (ret.second == false) {
        error.addMeesage("OpenCL-Kernel with  '" + kernel + "' already exist");
        return false;
    }
 ...
 * @param kernel,  of the kernel, which should be used
 * @param buffer  of buffer, which should be bind to the kernel
 * @param error reference for error-output
 *
 * @return true, if successful, else false
 */
...
GpuInterface::bindKernelToBuffer(GpuData& data,
                                 ...
                                 ErrorContainer& error)
{
    LOG_DEBUG("bind buffer with  '" + buffer + "' kernel with : '" + kernel + "'");

    // get kernel-data
    if (data.containsKernel(kernel) == false) {
        error.addMeesage("no kernel with  '" + kernel + "' found");
        return false;
    }

    // check if buffer- exist
    if (data.containsBuffer(buffer) == false) {
        ErrorContainer error;
        error.addMeesage("no buffer with  '" + buffer + "' found");
        return false;
    }

    // get kernel
    GpuData::KernelDef* def = data.getKernel(kernel);
    if (def == nullptr) {
        error.addMeesage("Kernel with  '" + kernel + "' not found.");
        return false;
    }

    // get buffer to bind to kernel
    GpuData::WorkerBuffer* buffer = &data.m_buffer[buffer];
    if (buffer == nullptr) {
        error.addMeesage("Buffer with  '" + buffer + "' not found.");
        return false;
    }
    ...
    LOG_DEBUG("bind buffer with  '" + buffer + "' to argument number "
    ...
    auto ret = def->arguments.try_emplace(buffer, argNumber);
    if (ret.second == false) {
        error.addMeesage("OpenCL-Argument with  '" + buffer + "' is already set.");
        return false;
    }
 ...
 *
 * @param data object with all data
 * @param kernel,  of the kernel, which should be executed
 ...
 *
 * @param data object with all data
 * @param buffer  of the buffer in the kernel
 * @param error reference for error-output
                                   ...
                                   const uint64_t offset)
{
    ...
    GpuData::WorkerBuffer* buffer = data.getBuffer(buffer);
    ...
    if (buffer->useHostPtr == false && numberOfObjects != 0) {
        ...
        if (m_queue.enqueueWriteBuffer(buffer->clBuffer,
            ...
            != CL_SUCCESS)
        {
            error.addMeesage("Update buffer with  '" + buffer + "' on gpu failed");
            return false;
        }
    }
 ...
 *
 * @param data object with all data
 * @param buffer  of the buffer to copy into
 * @param error reference for error-output
 *
 * @return true, if successful, else false
 */
bool
GpuInterface::copyFromDevice(GpuData& data, const std::string& buffer, ErrorContainer& error)
{
    // check id
    if (data.containsBuffer(buffer) == false) {
        error.addMeesage("no buffer with  '" + buffer + "' found");
...
/**
 * @brief GpuInterface::getDevice
 * @return
 */
const std::string
GpuInterface::getDevice()
{
    return m_device.getInfo<CL_DEVICE_>();
}
...
GpuInterface::closeDevice(GpuData& data)
{
    ...
    for (auto& [, workerBuffer] : data.m_buffer) {
        if (workerBuffer.data != nullptr && workerBuffer.allowBufferDeleteAfterClose) {
            Axon::alignedFree(workerBuffer.data, workerBuffer.numberOfBytes);
        }
    }
