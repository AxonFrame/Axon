...
SimpleTest::SimpleTest() : Axon::SpeedTestHelper()
{
    m_copyToDeviceTimeSlot.unit = "ms";
    m_copyToDeviceTimeSlot. = "copy to device";

    m_initKernelTimeSlot.unit = "ms";
    m_initKernelTimeSlot. = "init kernel";

    m_runTimeSlot.unit = "ms";
    m_runTimeSlot. = "run test";

    m_updateTimeSlot.unit = "ms";
    m_updateTimeSlot. = "update data on device";

    m_copyToHostTimeSlot.unit = "ms";
    m_copyToHostTimeSlot. = "copy to host";

    m_cleanupTimeSlot.unit = "ms";
    m_cleanupTimeSlot. = "cleanup";

    m_oclHandler = new Axon::GpuHandler();
...
SimpleTest::simple_test()
{
    ...
    Axon::GpuInterface* ocl = oclHandler.m_interfaces.at(m_id);
...
SimpleTest::chooseDevice()
{
    ...
    for (uint32_t i = 0; i < m_oclHandler->m_interfaces.size(); i++) {
        std::cout << "    " << i << ": " << m_oclHandler->m_interfaces.at(i)->getDevice()
                  << std::endl;
    }
