...
class GpuData
{
   public:
    ...
    bool addBuffer(const std::string& ,
                   const uint64_t numberOfObjects,
                   const uint64_t objectSize,
                   const bool useHostPtr = false,
                   void* data = nullptr);
    bool addValue(const std::string& , const uint64_t value);

    bool containsBuffer(const std::string& );
    void* getBufferData(const std::string& );
   ...
   private:
    ...
    WorkerBuffer* getBuffer(const std::string& );

    bool containsKernel(const std::string& );
    KernelDef* getKernel(const std::string& );

    uint32_t getArgPosition(KernelDef* kernelDef, const std::string& buffer);
};
