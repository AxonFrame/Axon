...
class TemplateSocket : public AbstractSocket
{
   public:
     ...
     * @param thread  of the thread of the socket
     */
    TemplateSocket(T&& socket, const std::string& thread) : AbstractSocket(thread)
    {
     ...
     */
    bool initConnection(Axon::ErrorContainer& error) { return m_socket.initClientSide(error); }
   ...
   private:
    ...
    bool waitForMessage()
    {
        ...
        const uint64_t writePosition = Axon::getWritePosition_RingBuffer(m_recvBuffer);
        const uint64_t spaceToEnd = Axon::getSpaceToEnd_RingBuffer(m_recvBuffer);
