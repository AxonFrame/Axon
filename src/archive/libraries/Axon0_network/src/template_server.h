...
class TemplateServer : public AbstractServer
{
   public:
     ...
     * @param thread  of the thread of the server
     */
    TemplateServer(T&& server,
                   void* target,
                   void (*processConnection)(void*, AbstractSocket*),
                   const std::string& thread)
        : AbstractServer(target, processConnection, thread)
    {
   ...
   protected:
     ...
     * @return true, if successful, else false
     */
    bool waitForIncomingConnections(Axon::ErrorContainer& error)
    {
        ...
        if (std::is_same<T, TcpServer>::value) {
            ...
            TemplateSocket<TlsTcpSocket>* netSocket
                = new TemplateSocket<TlsTcpSocket>(std::move(tlsTcpSocket), );
            netSocket->initConnection(error);
            m_processConnection(m_target, netSocket);
        }
