...
#include <Axon_common/methods/string_methods.h>
#include <Axon_common/methods/vector_methods.h>
...
ObjParser::parse(ObjItem& result, const std::string& inputString, ErrorContainer& error)
{
    ...
    Axon::replaceSubstring(preparedString, "\t", " ");

    // split string into the single lines
    std::vector<std::string> splittedContent;
    Axon::splitStringByDelimiter(splittedContent, preparedString, '\n');
    ...
    for (uint64_t i = 0; i < splittedContent.size(); i++) {
        ...
        Axon::splitStringByDelimiter(splittedLine, splittedContent.at(i), ' ');
        Axon::removeEmptyStrings(splittedLine);
...
ObjParser::parseIndexList(std::vector<Index>& result, const std::vector<std::string>& lineContent)
{
    ...
    for (uint32_t i = 1; i < lineContent.size(); i++) {
        ...
        Axon::splitStringByDelimiter(indexList, lineContent.at(i), '/');
