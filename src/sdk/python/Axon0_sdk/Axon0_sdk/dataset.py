...
from . import Axon_request
from . import Axon_exceptions
from .Axon_messages import proto3_pb2
import json
import math
import time
import asyncio
...
def list_datasets(token: str,
    ...
    return Axon_request.send_get_request(token,
                                           address,
                                           path,
                                           "",
...
def delete_dataset(token: str,
    ...
    return Axon_request.send_delete_request(token,
                                              address,
                                              path,
                                              values,
                                              verify=verify_connection)
...
def wait_until_upload_complete(token: str,
    ...
    while True:
        ...
        result = Axon_request.send_get_request(token,
                                                 address,
                                                 path,
                                                 values,
                                                 verify=verify_connection)
...
def upload_mnist_files(token: str,
    ...
    result = Axon_request.send_post_request(
    ...
    body_str = json.dumps(json_body)
    result = Axon_request.send_put_request(
        token, address, path, body_str, verify=verify_connection)
    result_json = json.loads(result)
...
def upload_csv_files(token: str,
    ...
    body_str = json.dumps(json_body)
    result = Axon_request.send_post_request(token,
                                              address,
                                              path,
                                              body_str,
                                              verify=verify_connection)
